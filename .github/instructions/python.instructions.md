...
- Files: `snake_case.py`
- Classes: `PascalCase` prefixed with `InstitutoHidrogrfico`
...
- Inherit from both platform entity and `InstitutoHidrogrficoEntity` (order matters)
...
from custom_components.hidrograficopt.sensor import async_setup_entry
...
**Config entry data:** `entry_data: InstitutoHidrogrficoData = hass.data[DOMAIN][entry.entry_id]`
