...
This document describes the technical architecture of the Instituto Hidrográfico Integration custom component for Home Assistant.
...
│   ├── __init__.py          # Exports InstitutoHidrogrficoDataUpdateCoordinator
...
│   ├── __init__.py          # Exports InstitutoHidrogrficoEntity
...
- `base.py` - Main coordinator class (`InstitutoHidrogrficoDataUpdateCoordinator`)
...
**Key class:** `InstitutoHidrogrficoDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `InstitutoHidrogrficoApiClient`
...
- `InstitutoHidrogrficoConfigFlowHandler` (main flow)
- `InstitutoHidrogrficoOptionsFlow` (options)
...
**Key class:** `InstitutoHidrogrficoEntity` (in `entity/base.py`)
...
2. `InstitutoHidrogrficoEntity` for common functionality
...
1. Create directory: `custom_components/hidrograficopt/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `InstitutoHidrogrficoEntity`
