...
This document describes all configuration options and settings available in the Instituto Hidrográfico Integration custom integration.
...
### `hidrograficopt.example_service`
...
```yaml
service: hidrograficopt.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: hidrograficopt.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
```yaml
blueprint:
  name: Instituto Hidrográfico Integration Alert
...
- [GitHub Issues](https://github.com/zewelor/hass-hidrograficopt/issues) - Report problems
